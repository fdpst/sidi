<template>
    <v-card class="pa-3 ma-3">
        <v-toolbar flat color="#1d2735" dark>
            <v-icon class="white--text" style="font-size: 45px">mdi-microsoft-powerpoint</v-icon>
            <pre><v-toolbar-title><h2>Lista Proyectos</h2></v-toolbar-title></pre>
        </v-toolbar>
        <loader v-if="isloading"></loader>
        <v-tooltip right>
            <template v-slot:activator="{ on, attrs }">
                <v-btn fab :to="{ path: `/guardar-proyecto` }" :loading="isloading" :disabled="isloading" color="orange darken-1" class="mt-2" v-bind="attrs" v-on="on">
        ...
        <v-data-table dense :headers="headers" :items="proyectos" :search="search" :items-per-page="15" item-key="id" class="elevation-0" :sort-by="['usuario.nombre']" :sort-desc="[false]">
            ...
            <template v-slot:item.action="{ item }">
                ...
                <router-link :to="{ path: `/guardar-proyecto?id=${item.id}` }" class="action-buttons">
        ...
        <v-dialog v-model="dialog" max-width="500px">
            <v-card>
                ...
                <v-card-actions class="pt-3">
                    ...
                    <v-btn color="success" large @click="deleteProyecto()">Confirmar</v-btn>
                    <v-spacer></v-spacer>
                </v-card-actions>
            </v-card>
        </v-dialog>
    ...
    export default {
        ...
        created() {
            if (this.$route.query.id) {
                this.getProyectosByUser(this.$route.query.id);
            }else{
        ...
        methods: {
            ...
            getProyectosByUser(userId) {
                axios.get(`api/get-proyectos-by-user-id/${userId}`).then(
                    (res) => {
            ...
            deleteProyecto() {
                this.dialog = false
                axios.get(`api/delete-proyecto/${this.proyectos[this.selectedItem].id}`).then(
                    (res) => {
