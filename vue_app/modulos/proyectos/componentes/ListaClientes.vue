<template>
    <v-card class="pa-3 ma-3">
        <v-toolbar flat color="#1d2735" dark>
            <v-icon class="white--text" style="font-size: 45px;">mdi-account-supervisor-circle</v-icon><pre><v-toolbar-title><h2>Lista Clientes</h2></v-toolbar-title></pre>
        </v-toolbar>
        <loader v-if="isloading"></loader>
        <v-tooltip right>
            <template v-slot:activator="{ on, attrs }">
                <v-btn fab :to="{ path: `/guardar-usuario` }" :loading="isloading" :disabled="isloading" color="orange darken-1" class="mt-2" v-bind="attrs" v-on="on">
        ...
        <v-data-table dense :headers="headers" :items="Usuarios" :search="search" :items-per-page="15" item-key="id" class="elevation-1" :sort-by="['nombre']" :sort-desc="[false]">
            <template v-slot:item.fecha_alta="{ item }">
                <span>{{ item.fecha_alta }}</span>
            </template>
            <template v-slot:item.action="{ item }">
                <router-link :to="{ path: `/lista-proyectos?id=${item.id}` }" class="action-buttons">
                    <v-icon small class="mr-2" color="green" style="font-size: 25px;" title="EDITAR">mdi-eye</v-icon>
                </router-link>
            ...
            <v-card>
                ...
                <v-card-actions class="pt-3">
                    ...
                    <v-btn
                        color="success"
                        large
                        @click="deleteUser()">Confirmar</v-btn>
                    <v-spacer></v-spacer>
                </v-card-actions>
            </v-card>
        </v-dialog>
...
<script>
    export default {
        ...
        methods: {
            getClientes() {
                axios.get(`api/get-usuarios-clientes`).then(res => {
            ...
            deleteUser(){
                axios.get(`api/delete-usuario/${this.Usuarios[this.selectedItem].id}`).then(res => {
